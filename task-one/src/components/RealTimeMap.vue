...
<script setup>
import { reactive, toRefs, getCurrentInstance, onMounted, watch, inject } from 'vue'
import "echarts-gl"
...
watch(rtMapData, (val) => {
    chartObj.setOption({
        series: [
            {
                data: convertData(val.data),
            },
        ],
    });
})
onMounted(() => {
    var myChartDom = document.getElementById("rtmContainer");
    ...
    $http
        ...
        .then((res) => {
            ...
            proxy.$echarts.registerMap("xiangtanshi", geoJson);
            myChart.hideLoading();
            let option = {
                title: {
                    text: "湘潭市疫情密度地图",
                ...
                geo3D: {
                    ...
                    environment: new proxy.$echarts.graphic.LinearGradient(
                        0,
                        0,
                        0,
                        1,

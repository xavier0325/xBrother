<template>
    <div>
        <div id="allmap"></div>
        <div id="mapDiv">
            <a>已完成节能改造项目：<span>{{ mapDivData.projectNum }}</span> 个</a>
            <a>已完成节能改造面积：<span>{{ mapDivData.projectArea }}</span>
                万平方米</a>
            <a>申报项目消耗能源合计：<span>{{ mapDivData.consumptionTotal }}</span>
                吨标准煤</a>
            <a>申报项目平均节能率：<span>{{ mapDivData.rateEnergySaving }}</span>%</a>
        </div>
        <div id="mapDiv1">
...
onMounted(() => {
    // 百度地图API功能
    let bmap = document.getElementById("allmap");
    bmap.style.height = window.innerHeight * 0.9 + "px";
    // bmap.style.width = window.innerWidth * 0.5 + "px";
    ...
    $http
        ...
        .then(function (response) {
            ...
            mapDivData.value = response.data.projectmapData;
            let points = [...mapDivData.value.projects.points];
            addMarker(points);
        })
    ...
    function addMarker(points) {
        ...
        for (let i = 0; i < points.length; i++) {
            ...
            switch (points[i].type) {
                case 1:
                    myIcon = new BMap.Icon(icon1, new BMap.Size(40, 45));
                    break;
                case 2:
                    myIcon = new BMap.Icon(icon2, new BMap.Size(40, 45));
                    break;
                case 3:
                    myIcon = new BMap.Icon(icon3, new BMap.Size(40, 45));
                    break;
                case 4:
                    myIcon = new BMap.Icon(icon4, new BMap.Size(40, 45));
                    break;
                default:
                    break;
            }
    ...
    function showInfo(thisMarker, point) {
        //axios请求数据
        let projectName = point.project_name;
        let buildingName = point.building_name;
        let projectAddr = point.project_addr;
        let buildingStruct = point.building_struct;
        let startDay = point.start_day;
        let investSum = point.invest_sum;
        let transformEySaving = point.transform_eySaving;
        let buildingFunc = point.building_func;
        let buildingAreaTotal = point.building_areaTotal;
        let floorAreaTotal = point.floor_areaTotal;
        let endDay = point.end_day;
        let useDay = point.use_day;
        let contractEyUsed = point.contract_eyUsed;
        let rateEyTransform = point.rate_eyTransform;
        let imgUrl = point.img_url;
        let powerBefore = point.power_before;
        let powerAfter = point.power_after;
        ...
        infoWindow.addEventListener("open", function () {
            let el = document.getElementById("mapDiv");
            let el1 = document.getElementById("mapDiv1");
            let echartsOne = document.getElementById("containerOne");
            let echartsTwo = document.getElementById("containerTwo");

            map.centerAndZoom(new BMap.Point(point.lng - 0.02, point.lat), 14)
            ...
            let eyConsumption = proxy.$echarts.init(echartsOne);
            let eyConsumption2 = proxy.$echarts.init(echartsTwo);
